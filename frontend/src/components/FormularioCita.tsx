...
export default function AgendarCita() {
  ...
  const { listaMascotas, listaVeterinarios, cargandoCatalogos } = useCatalogos(); 
  ...
  if (cargandoCatalogos) {
    return <div className="p-6 text-center text-gray-500 animate-pulse">Cargando datos del sistema...</div>;
  }

  return (
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 py-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className={`w-full border rounded-lg p-2 outline-none ${rol === 'veterinario' ? 'bg-gray-100 text-gray-500 border-gray-300 cursor-not-allowed' : 'bg-white text-black border-gray-300 focus:ring-2 focus:ring-blue-500'}`}
            >
              <option value="">-- Selecciona Médico --</option>
              {listaVeterinarios.map((v) => (
                <option key={v.id} value={v.id}>Dr/a. {v.nombre}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full border text-gray-500 border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none"
          />
        </div>
